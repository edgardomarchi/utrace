...
if USE_JAX:
    ...
    def _masked_quantile_higher(sorted_padded: jnp.ndarray,
        ...
        """Quantile with method='higher' over the first n_valid entries of
        `sorted_padded`, which must be sorted ascending with +inf padding after
        position n_valid.

        Shape-stable: operates on the full (max_N,) array, so JAX compiles it once
        regardless of n_valid. Replicates jnp.quantile(sorted_padded[:n_valid], q,
        method='higher') exactly (verified by test_masked_quantile_matches_jnp).
        """
        pos = q * (n_valid - 1)
