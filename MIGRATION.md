...
- precompute_proba helper: the per-(noise/split) "forward the model over a loader, collect probs+labels" loop is duplicated across scripts (a standalone version in convergence_analysis, inline variants in the MNIST scripts; they differ — some return torch tensors uncut for zero-copy, others cat into lists). Consolidate into a scripts/_common.py helper. Design must settle: return a concatenated tensor vs list, and whether it handles the label .numpy() (couples with the labels-.numpy() removal task).
...
- DataLoader num_workers>0 uses fork by default, which deadlocks with JAX (multithreaded): "os.fork() ... incompatible with multithreaded code". Plan: use spawn (multiprocessing_context='spawn' per loader, or set_start_method) so workers remain usable. Note: spawn requires picklable transforms (the lambda->AddGaussianNoise migration already satisfies this) and the __main__ guard (scripts already have it).
